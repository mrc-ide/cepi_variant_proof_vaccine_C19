...
seeding_cases <- 10
matrix_primary<- "./data/HIC_coverage_primary.csv"
matrix_d3 <-  "./data/HIC_coverage_primary.csv"
...
#vaccine_doses == 3 & omicron_vaccine == 0 )| vaccine_doses == 6 ) %>%
...
# Run
runs3 <- run$enqueue_bulk(scenarios, run_scenario, do_call = TRUE, progress = TRUE)
runs3$status()
